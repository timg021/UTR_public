...
The theory behind the UTR algorithm is described in the paper by T.E. Gureyev, H.G. Brown, H.M. Quiney and L.J. Allen, 
Fast unified reconstruction algorithm for conventional, phase-contrast and diffraction tomography, eprint arXiv: 2206.09151.
(https://arxiv.org/abs/2206.09151). 
...
7.Width_and_height_of_input_images_in_pixels_(images_from_files_can_be_trimmed_or_padded_with_1.0): 2304 640
8.Pixel_size_in_microns: 100
9.Centre_of_rotation_x,_y_and_z_shifts_in_microns: 400 0 0 
10.Wavelength_in_microns: 0.000038745052
11.Phase_retrieval_method:_none(0),_TIE-Hom-2D(1),_TIE-Hom-3D(2)_CTF-2D(3)_or_CTF-3D(4): 2
12.Apply_Ewald_sphere_curvature_correction:_No(0)_or_Yes(1): 0
13.Delta/beta_ratio_(NOTE:_n=1-delta+i*beta): 275.0
14.Average_noise-to-signal_ratio(1/SNR)_in_input_images: 0.15
...
The text file given in Parameter 3 must be present in the same folder that UTR.exe is started from,
or, alternatively, the filename can include a fully specified pathname (OS specific). 

Parameter 4 contains the pathname of an optional file with indexes and weights for subseries selection or NONE,
...
(from the set defined by Parameter 3) for processing in this program. The second column should contain relative 
...
the effective dose can be halved. In the case of cryo-EM imaging, this parameter can be used to weighting the input
image frames in accordance with the degree of their correlation with the calculated defocused images (at the same orientation
...
"0" corresponds to no phase retrieval used, i.e. the CT reconstruction is performed directly from the input images without any
phase retrieval or CTF correction. This corresponds to conventional CT reconstruction.
...
also in the inverse Laplace filtering, when Parameter 22 is set to 1. Typical values of this parameter are between 0.1 (strong
regularization) and 0.0001 (weak regularization). If this parameter is equal to zero, no regularization is applied. The zero
value of this parameter is usually safe to use with the TIE phase retrieval methods (i.e. when Parameter 15 is equal to 1 or 2),
but not with the CTF correction methods (i.e. when Parameter 15 is equal to 3 or 4).
...
range [0, max-out] according to the linear transformation: max-out * (V(i,j) - min-in) / (max-in - min-in). Note that
since the re-scaled valued are subsequently saved in 16-bit TIFF files, the max-out value should be between 0 and 65,535.
...
======================================================================================= 
** Example of a text file with ".relionnew" extension whose name may appear in Parameter 3. All numeric parameter values are given either in Units of Length or degrees.
...
======================================================================================= 
** Example of a text file with ".relionnew" extension whose name may appear in Parameter 4.
6604	2.90E-02
2962	2.86E-02
9268	2.74E-02
9337	2.73E-02
